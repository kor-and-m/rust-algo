use graph_representations::{AdjacencyListGraphIncome, AdjacencyListGraphOutcome, Edge, SimpleGraph};
...
pub fn jonsons(graph: &mut SimpleGraph) -> Option<Vec<Vec<isize>>> {
    ...
    for i in 0..last_idx {
        graph.add_edge(Edge {from: last_idx, to: i, length: 0})
    };
...
mod tests {
    ...
    fn graph_from_file(path: &str) -> SimpleGraph {
        let file =
          File::open(path).expect("Something went wrong reading the file");
