...
fn knapsack_step(
    ...
    size: usize,
    initial_size: usize,
    results: &mut HashMap<usize, Vec<usize>>,
) -> usize {
    if copasity == 0 {
        return 0;
    }
    ...
    let calc_deep = {
        let current_copasity_memory = results
            .entry(copasity_idx)
            .or_insert(Vec::with_capacity(initial_size));
        let mut calc_deep = current_copasity_memory.len();

        if calc_deep == 0 {
            let item = &items[0];
            current_copasity_memory.push(if copasity >= item.weight { item.val } else { 0 });
            calc_deep += 1;
        }
        calc_deep
    };
