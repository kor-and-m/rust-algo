...
fn split_array<T: Copy + Ord + std::fmt::Debug + Hash>(n: Vec<T>, history: &mut HashMap<T, T>, counter: &mut usize) -> Vec<T> {
    let l = n.len();
...
fn find_max_pair<T: Copy + Ord + std::fmt::Debug + Hash>(n: Vec<T>, counter: &mut usize) -> (T, T) {
    ...
    if l == 2 {
        return if n[0] > n[1] {(n[0], n[1])} else {(n[1], n[0])}
    }
...
pub fn find_second_max_safe<T: Copy + Ord + std::fmt::Debug + Hash>(n: Vec<T>) -> (Option<T>, usize) {
