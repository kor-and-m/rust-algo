...
mod tests {
    use std::fs;
    use super::{quicksort, median, first, last};
    ...
    #[test]
    fn it_works_2() {
        let mut v = [42, 80, 92, 19, 26, 23, 42, 62, 32, 23, 95, 1, 1, 88, 17, 37, 3, 24, 95, 53];
        let mut v2= v.clone();
        quicksort(&mut v, &first);
        v2.sort();
        check(&v, &v2)
    ...
    fn it_works_file_median() {
        ...
        let mut v: Vec<i32> = contents.split_whitespace().map(|x| x.parse::<i32>().unwrap()).collect();
        let mut v2 = v.clone();
