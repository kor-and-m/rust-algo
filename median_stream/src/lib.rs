...
impl TwoHeaps {
    pub fn new() -> Self {
        TwoHeaps { top: BinaryHeap::new(), bottom: BinaryHeap::new() }
    }
}
...
impl Medianable for TwoHeaps {
    fn update_state(&mut self, num: usize) {
        match (self.bottom.peek(), self.bottom.len(), self.top.len()) {
            ...
            (Some(v), b_len, t_len) if *v < num as isize && b_len > t_len => { self.top.push(-(num as isize)) },
    ...
    fn calculate_median(&self) -> f64 {
        match (self.bottom.len(), self.top.len(), self.bottom.peek(), self.top.peek()) {
